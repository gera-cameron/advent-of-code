...
main = do
  ...
  print
    . (\((gId, minute), _) -> gId * minute)
    . last
    . List.sortOn snd
    . Map.toList
    . findGuardSleepFrequencies
    . fmap (calculateTimeAsleep [])
    . buildGuardToActionMap [] Nothing
    . fmap snd
    . List.sortOn fst
    . Maybe.catMaybes
    . fmap (buildTimeToEventTuple . words)
...
findGuardSleepFrequencies :: Map.Map Int [Int] -> Map.Map (Int, Int) Int
findGuardSleepFrequencies = Map.fromListWith (+) . concatMap (\(key, minutes) -> fmap (\minute -> ((key, minute), 1)) minutes) . Map.toList
